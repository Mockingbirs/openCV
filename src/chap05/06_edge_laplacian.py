...
cv2.imshow("filter2D 4, dst1", cv2.convertScaleAbs(dst1))
cv2.imshow("filter2D 8, dst2", cv2.convertScaleAbs(dst2))
cv2.imshow("Laplacian_OpenCV, dst3", cv2.convertScaleAbs(dst3))
cv2.waitKey(0)
